...
export default function QuotationBuilder() {
  ...
  const calculations = useMemo(() => {
    ...
    let discountAmount = Number(watchedValues.discount) || 0;

    // Get pricing from selected package (one-time charges per person)
    const selectedPackage = pricingPackages?.find(p => p.id === watchedValues.pricingPackageId);
    const adultPackageCharge = selectedPackage ? Number(selectedPackage.adultRate) : 0;
    const childPackageCharge = selectedPackage ? Number(selectedPackage.childRate) : 0;
    const infantPackageCharge = selectedPackage ? Number(selectedPackage.infantRate) : 0;

    // Makkah Hotel - Calculate per-person rate based on room category
    if (watchedValues.makkahDays > 0 && watchedValues.makkahHotelId) {
      const makkahHotel = makkahHotels.find(h => h.id === watchedValues.makkahHotelId);
      if (makkahHotel) {
        const bedCount = Number(watchedValues.makkahRoomBeds) || 2;
        let roomPrice = 0;
        
        if (watchedValues.makkahRoomBeds === "2") roomPrice = Number(makkahHotel.price2Bed);
        else if (watchedValues.makkahRoomBeds === "3") roomPrice = Number(makkahHotel.price3Bed);
        else if (watchedValues.makkahRoomBeds === "4") roomPrice = Number(makkahHotel.price4Bed);
        else if (watchedValues.makkahRoomBeds === "5") roomPrice = Number(makkahHotel.price5Bed);
        
        const perPersonRate = roomPrice / bedCount;
        const totalPeople = watchedValues.adults + watchedValues.children + watchedValues.infants;
        hotelCostMakkah = perPersonRate * totalPeople * watchedValues.makkahDays;
      }
    }

    // Madina Hotel - Calculate per-person rate based on room category
    if (watchedValues.madinaDays > 0 && watchedValues.madinaHotelId) {
      const madinaHotel = madinaHotels.find(h => h.id === watchedValues.madinaHotelId);
      if (madinaHotel) {
        const bedCount = Number(watchedValues.madinaRoomBeds) || 2;
        let roomPrice = 0;
        
        if (watchedValues.madinaRoomBeds === "2") roomPrice = Number(madinaHotel.price2Bed);
        else if (watchedValues.madinaRoomBeds === "3") roomPrice = Number(madinaHotel.price3Bed);
        else if (watchedValues.madinaRoomBeds === "4") roomPrice = Number(madinaHotel.price4Bed);
        else if (watchedValues.madinaRoomBeds === "5") roomPrice = Number(madinaHotel.price5Bed);
        
        const perPersonRate = roomPrice / bedCount;
        const totalPeople = watchedValues.adults + watchedValues.children + watchedValues.infants;
        hotelCostMadina = perPersonRate * totalPeople * watchedValues.madinaDays;
      }
    }

    // Package charges - one-time per person (NOT multiplied by days)
    let packageCharges = 0;
    packageCharges += adultPackageCharge * watchedValues.adults;
    packageCharges += childPackageCharge * watchedValues.children;
    packageCharges += infantPackageCharge * watchedValues.infants;

    // Makkah Meals
    const selectedMakkahMeal = activeMeals.find(m => m.name === watchedValues.makkahMealPlan);
    if (selectedMakkahMeal && watchedValues.makkahDays > 0) {
      const mealRate = Number(selectedMakkahMeal.pricePerDay);
      mealCostMakkah = (watchedValues.adults + watchedValues.children + watchedValues.infants) * mealRate * watchedValues.makkahDays;
    }

    // Madina Meals
    const selectedMadinaMeal = activeMeals.find(m => m.name === watchedValues.madinaMealPlan);
    if (selectedMadinaMeal && watchedValues.madinaDays > 0) {
      const mealRate = Number(selectedMadinaMeal.pricePerDay);
      mealCostMadina = (watchedValues.adults + watchedValues.children + watchedValues.infants) * mealRate * watchedValues.madinaDays;
    }

    // Laundry
    if (activeLaundry.length > 0) {
      const laundryRate = Number(activeLaundry[0].pricePerDay);
      if (watchedValues.makkahLaundry) {
        laundryCost += laundryRate * watchedValues.makkahDays * payingTravelers;
      }
      if (watchedValues.madinaLaundry) {
        laundryCost += laundryRate * watchedValues.madinaDays * payingTravelers;
      }
    }

    // Transport
    selectedTransport.forEach(st => {
      const transport = activeTransport.find(t => t.id === st.transportId);
      if (transport) {
    ...
    if (selectedVisa) {
      visaCost = Number(selectedVisa.price) * payingTravelers;
    }

    // Add-ons
    selectedAddons.forEach(sa => {
      const addon = activeAddons.find(a => a.id === sa.addonId);
      if (addon) {
    ...
    serviceCharges?.forEach(sc => {
      ...
      if (sc.appliesTo === "all" || sc.appliesTo === "adult") {
        serviceChargeCost += amount * watchedValues.adults;
      }
      if (sc.appliesTo === "all" || sc.appliesTo === "child") {
        serviceChargeCost += amount * watchedValues.children;
      }
    });

    // Total calculation (INR only)
    ...
    const totalInr = Math.max(0, subtotal - discountAmount);
    const perPersonInr = payingTravelers > 0 ? totalInr / payingTravelers : 0;
  ...
  }, [watchedValues, pricingPackages, makkahHotels, madinaHotels, activeMeals, activeLaundry, activeTransport, activeVisa, activeAddons, serviceCharges, selectedTransport, selectedAddons, payingTravelers]);
                              ...
                              <SelectContent>
                                {makkahHotels.map(hotel => (
                                  <SelectItem key={hotel.id} value={hotel.id.toString()}>
                                    {hotel.name} ({hotel.starRating}⭐) - SAR {Number(hotel.pricePerNight)}/night
                                  </SelectItem>
                                ))}
                              </SelectContent>
                            </Select>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="space-y-3">
                      <div className="rounded-lg bg-primary/10 p-4">
                        ...
                        <p className="text-2xl font-bold font-mono" data-testid="text-total-inr">
                      ...
                      <div className="rounded-lg border p-4">
                        ...
                        <p className="text-xl font-bold font-mono" data-testid="text-per-person-inr">
