...
import type { Quotation, Hotel, Visa, Meal, Laundry, PricingPackage } from "@shared/schema";
...
export default function QuotationView() {
  ...
  const calculatedPricing = useMemo(() => {
    ...
    if (makkahHotel && quotation.makkahDays && quotation.makkahDays > 0) {
      const roomPrice = Number(makkahHotel.price2Bed) || 0;
      const perPersonRate = roomPrice / 2;
      hotelCostMakkah = perPersonRate * totalPeople * quotation.makkahDays;
    }

    if (madinaHotel && quotation.madinaDays && quotation.madinaDays > 0) {
      const roomPrice = Number(madinaHotel.price2Bed) || 0;
      const perPersonRate = roomPrice / 2;
    ...
    const subtotal = hotelCostMakkah + hotelCostMadina + packageCharges + mealCostMakkah + mealCostMadina + laundryCost + visaCost;
    const total = Math.max(0, subtotal - discountAmount);
    const perPerson = payingTravelers > 0 ? total / payingTravelers : 0;

    return { total, perPerson };
  }, [quotation, makkahHotel, madinaHotel, visa, pricingPackages, meals, laundryList]);
  ...
  const handleSendWhatsApp = () => {
    ...
    const message = generateWhatsAppMessage(quotation, makkahHotel, madinaHotel, visa);
            ...
            <Card>
              ...
              <CardContent className="space-y-6">
                <div className="rounded-lg bg-primary/10 p-4">
                  ...
                  <p className="text-2xl font-bold font-mono" data-testid="text-total-sar">
                ...
                <div className="rounded-lg border p-4">
                  ...
                  <p className="text-xl font-bold font-mono" data-testid="text-per-person-sar">
